...
class SRW_RWF_ISRW:
    ...
    def random_walk_sampling_simple(self, complete_graph, nodes_to_sample):
        complete_graph = nx.convert_node_labels_to_integers(
    ...
    def random_walk_sampling_with_fly_back(
        self, complete_graph, nodes_to_sample, fly_back_prob
    ):
        complete_graph = nx.convert_node_labels_to_integers(
            complete_graph, 0, "default", True
        )
        ...
        while sampled_graph.number_of_nodes() != upper_bound_nr_nodes_to_sample:
            ...
            choice = np.random.choice(
                ["prev", "neigh"], 1, p=[fly_back_prob, 1 - fly_back_prob]
            )
            if choice == "neigh":
                curr_node = chosen_node
            iteration = iteration + 1
            ...
            if iteration % self.T == 0:
                if (
                    sampled_graph.number_of_edges() - edges_before_t_iter
                ) < self.growth_size:
                    curr_node = random.randint(0, nr_nodes - 1)
